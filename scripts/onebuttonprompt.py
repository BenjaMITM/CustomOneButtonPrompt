...
basemodellist = ["SD!.5", "SDXL", "Stable Cascade", "Anime Model"]
...
artists = ["all","none"]
imagetypes = ["all","photograph","octane render", "digital art", "concept", "painting", "portrait"]
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        with gr.Tab("Main"):
            with gr.Row():
                insanitylevel = gr.Slider(1, 10, value=5, step=1, label="Higher levels increses complexity and randomness of generated prompt")
            with gr.Row():
                with gr.Column():
                    subject = gr.Dropdown(
                        subjects, label="Subject Types",
                        value="all")
                with gr.Column():
                    artist = gr.Dropdown(
            ...
            with gr.Row():
                gr.Markdown(
                    ...
                    Recommend is keeping it around 3-7. Use 10 at your own risk.
    ...
    def run(self, p, insanitylevel, subject, artist, imagetype):
        ...
        for i in range(batches):
            p.prompt = build_dynamic_prompt(insanitylevel, subject,artist, imagetype) + ", " + p.prompt
